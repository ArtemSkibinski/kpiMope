...
def calculate(m, n, q):
    ...
    print("\nAverage response function values:")
    ...
    print("y = {0} + {1}*x1 + {2}*x2 + {3}*x3".format(round(b0, 5), round(b1, 5), round(b2, 5), round(b3, 5)))
    ...
    g_t = table.get(m)
    if g_p < g_t:
        print(f"Homogeneous dispersion: Gp = {g_p:.5} < Gt = {g_t}")
    else:
        print(f"Non-homogeneous dispersion Gp = {g_p:.5} < Gt = {g_t}")
        m = m + 1
        calculate(m + 1, n, q)
        return
    ...
    print("Normalized values: ", ckeck_list)
    ...
    if f_p > f_t[f3][f4]:
        print(f"fp = {f_p} > ft = {f_t[f3][f4]}.\nMath model is non-adequate to experimental data")
    else:
        print(f"f_p = {f_p} < f_t = {f_t}.\nMath model is adequate to experimental data")
...
x_max_avg = (x1_max + x2_max + x3_max) / 3
x_min_avg = (x1_min + x2_min + x3_min) / 3
y_max = 200 + x_max_avg
y_min = 200 + x_min_avg
